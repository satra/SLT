function roi_render(dat,brt,rendfile)
% Render blobs on surface of a 'standard' brain
% FORMAT roi_render(dat,brt,rendfile)
%
...
%            spm_xbrain.m.
%
% Without arguments, roi_render acts as its own UI.
%_______________________________________________________________________
% 
% roi_render prompts for details of up to three SPM{Z}s or SPM{t}s that
...
if ~finite(brt),
	...
	for i=1:length(rend),
		...
		fprintf('Max is: %f',max(X{1}(:)));
		fprintf('Min is: %f',min(X{1}(:)));
		rgb = zeros([size(ren) 3]);
		dk = 0.7;
		tmp = dk.*ren.*max(1-X{1}-X{2}-X{3},0);
		rgb(:,:,1) = tmp + X{1};
		rgb(:,:,2) = tmp + X{1};
		rgb(:,:,3) = tmp + X{1};
		ax=axes('Parent',Fgraph,'units','normalized',...
