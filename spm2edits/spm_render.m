...
%=======================================================================
%
% Edited 01/26/05 (Jay Bohland) to add new render options
...
if nargin < 2,
	brt = 1;
	if num==1,
		brt = spm_input('Style',1,'new|old|gray',[1 NaN -1], 1);
		% Does the user want a grayscale version?
		isgrayscale = (brt==-1);
 		fprintf(sprintf('brt = %d, isgray = %d',brt,isgrayscale));
	end;
	if finite(brt),
...
% JB 3/10/05 - Jay still needs to fix this to make the grayscale
% figures work in all contexts
% SG 7/18/05 any time you provide a renderfile, the following lines
% change brt to -brt causing only the rendered parts of the brain
% to appear.
if (nargin > 2)
 isgrayscale = (brt<0);
 if isgrayscale,
...
for j=1:length(dat),
	...
	for i=1:length(rend),
		...
		% Insert some troubleshooting here to take care of some crashes when using Matlab 7.0.1
		% JB 02/28/05
	 	% size(d2)
		if ((size(d2,1) == 0) | (size(d2,2) < 2)), % no voxels survive thresh
		  error('Cannot render these results - no voxels in render overlay');
		  return;
 		end;
...
if ~finite(brt),
	...
	%---------------------------------------------------------------
		...
		if (isgrayscale),
		 tmp = 0.75*ren.*max(1-X{1}-X{2}-X{3},0);
		 rgb(:,:,1) = tmp + X{1};
                 rgb(:,:,2) = tmp + X{1};
                 rgb(:,:,3) = tmp + X{1};
                else
