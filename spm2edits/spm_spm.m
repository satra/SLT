...
% Updated 03/01/05 by Jay Bohland to fix Matlab 7.* warnings
% Changed lines like fprintf('%s%30s',sprintf('\b')*ones(1,30),'...done') to
% fprintf('%s%30s',repmat(sprintf('\b'),1,30),'...done') to account for 
% change in how Matlab deals with string computations
...
fprintf('DEBUG: Using modified spm_spm.m\n');
% [Satra] Preallocate large matrices
pack;
if ~isfield(xVi,'V')
    if size(Cy,1) == 1,
	Cy = zeros(nScan);
...
for z = 1:zdim				%-loop over planes (2D or 3D data)
    ...
    if strcmp(comp_type,'GLNX86')
      notdone = 1;
      % Determine MAXMEM on the fly
      %fprintf('[%s]Searching for maxmem\n',comp_type);
      MAXMEMnew = 2^31.89;
      while notdone
	try
          dummy = zeros(round(sqrt(MAXMEMnew/8)));
          clear dummy
          notdone = 0;
	catch
          MAXMEMnew = 2^(log2(MAXMEMnew)-0.5);
	end
      end
      %pack;
      %-block size
      blksz    = ceil(MAXMEMnew/8/nScan/max(ceil((round(zdim/2)-abs(round(zdim/2)-z))/2),1));
    end
    
    if strcmp(comp_type,'GLNXA64')
      %fprintf('[%s]Setting maxmem\n',comp_type);
      MAXMEMnew = 2^33.89;
      %-block size
      blksz    = ceil(MAXMEMnew/8/nScan);
    end

     %-# blocks
    nbch     = ceil(xdim*ydim/blksz);	
    
    % Significantly improves performance
    % [SG 13Jul06] 
    blksz = min(blksz,xdim*ydim);
		...
		if ~isfield(xVi,'V')
			...
			if length(j)
				...
				try
				    ...
				    frameSz = floor(nScan/6);
				    for Yloop=1:ceil(nScan/frameSz),
					fidx1 = ((Yloop-1)*frameSz+1);
					fidx2 = min(Yloop*frameSz, ...
						    nScan);
					blkid = [fidx1:fidx2]';
					Cy(blkid,:) = Cy(blkid,:)+Y(blkid,:)*Y';
				    end;
				end;
			end
...
if 0,
disp('[satra] Writing temporary analysis results file')
save('analysis_temp1');
disp('[satra] Finished writing temporary analysis results file')
end
...
%		xCon(end + 1) = spm_FcUtil('Set',Fcname,'F','iX0',iX0,xX.xKXs);
    ...
    disp('Warning: could not append all contrasts');
end
...
try
    ...
    disp('[satra]Could not compute scaled design matrix for display');
end

fprintf('%s%30s\n',repmat(sprintf('\b'),1,30),'...done')               %-#

if 0,
disp('[satra] Writing second temporary analysis results file')
save('analysis_temp2');
disp('[satra] Finished writing temporary analysis results file')
end
...
%-place fields in SPM
%-----------------------------------------------------------------------
SPM.xVi        = xVi; clear xVi;		% non-sphericity structure
SPM.xVi.CY     = CY; clear CY;			%-<(Y - <Y>)*(Y - <Y>)'> 
