...
%roidata = roidata.*(roidata<32000)+(roidata-32000).*(roidata>32000);
load(surffile);
...
for i=1:length(labels),
    ...
    if ~isempty(numverts),
        ...
        ROIpatch(i).cpatch = fv.vertices(vertidx(minidx),:);
        [vidx2,as1] = boundaryVertices(inflatedSurf,vertidx,faceids);
        ROIpatch(i).bvert = vertidx(find(vidx2));
