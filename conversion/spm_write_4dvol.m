...
if isfield(V,'private'),
    if isfield(V.private,'cdf'),
	idx = strmatch('time',{V.private.cdf.dim_array(:).name}');
	if ~isempty(idx),
	    time_pts = V.private.cdf.dim_array(idx).dim_length;
	    if time_pts>1,
		call_default = 0;
	    end;
	end;	
...
%if ~isfield(V,'cdf') | ~isfield(V.cdf,'time_pts') | V.cdf.time_pts
%== 1,
if call_default,
    Y = spm_read_vols(V);
    if nargin>1,
...
for ntime=1:NTime,
    V1 = spm_vol([V.fname,',',num2str(ntime)]);
    Y = spm_read_vols(V1);
    V1.fname = [pth,filesep,nm,sprintf(exttemplate,ntime),'.img'];
    spm_write_vol(V1,Y);
end;
